...
// ---------------------------------------------------------------------------
export async function humanMove(raw, startX, startY, endX, endY, cfg, signal) {
...
export async function humanClick(raw, isInput, cfg, signal) {
...
export async function humanIdle(raw, secondsOrCx, cxOrCy, cyOrCfg, maybeCfgOrSignal, maybeSignal) {
    const hasExplicitSeconds = maybeCfgOrSignal !== undefined
        && 'idle_between_duration' in maybeCfgOrSignal;
    const signal = hasExplicitSeconds ? maybeSignal : maybeCfgOrSignal;
    const seconds = hasExplicitSeconds
        ? secondsOrCx
        : rand(cyOrCfg.idle_between_duration[0], cyOrCfg.idle_between_duration[1]);
    const cx = hasExplicitSeconds ? cxOrCy : secondsOrCx;
    const cy = hasExplicitSeconds ? cyOrCfg : cxOrCy;
    const cfg = hasExplicitSeconds ? maybeCfgOrSignal : cyOrCfg;

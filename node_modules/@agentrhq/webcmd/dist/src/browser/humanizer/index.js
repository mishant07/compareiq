 ...
 */
import { mergeConfig, rand, randRange, sleep } from './config.js';
...
import { ensureActionable, ensureStable, checkPointerEvents, CHECKS_CLICK, CHECKS_HOVER, CHECKS_INPUT, CHECKS_FOCUS, CHECKS_CHECK, } from './actionability.js';
...
const patchedFrames = new WeakMap();
export function restorePatchedFrames(page) {
    for (const frame of patchedFrames.get(page) ?? []) {
        const descriptors = frame._humanRestoreDescriptors;
        if (!descriptors)
            continue;
        for (const [key, descriptor] of descriptors) {
            if (descriptor)
                Object.defineProperty(frame, key, descriptor);
            else
                Reflect.deleteProperty(frame, key);
        }
    }
...
async function selectorHumanTypeTarget(stealth, page, selector, sensitive) {
    if (stealth) {
        try {
            const escaped = JSON.stringify(selector);
 ...
 */
export function patchPage(page, cfg, cursor, signal) {
    const originals = {
    ...
    const humanHoverFn = async (selector, options) => {
        ...
        const remainingMs = () => Math.max(0, deadline - Date.now());
        if (!force && !skipChecks)
            await ensureActionable(page, selector, CHECKS_HOVER, remainingMs(), force);
        if (callCfg.idle_between_actions) {
            await humanIdle(raw, cursor.x, cursor.y, callCfg, signal);
        }
        const { box, cursorX, cursorY, didScroll } = await scrollToElement(page, raw, selector, cursor.x, cursor.y, callCfg, remainingMs(), signal);
    ...
    const humanTypeFn = async (selector, text, options) => {
        ...
        const remainingMs = () => Math.max(0, deadline - Date.now());
        if (!force)
            await ensureActionable(page, selector, CHECKS_INPUT, remainingMs(), force);
    ...
    const humanClearFn = async (selector, options) => {
        ...
        const remainingMs = () => Math.max(0, deadline - Date.now());
        if (!force)
            await ensureActionable(page, selector, CHECKS_FOCUS, remainingMs(), force);
    ...
    const humanCheckFn = async (selector, options) => {
        ...
        const remainingMs = () => Math.max(0, deadline - Date.now());
        if (!force)
            await ensureActionable(page, selector, CHECKS_CHECK, remainingMs(), force);
        if (callCfg.idle_between_actions) {
            await humanIdle(raw, cursor.x, cursor.y, callCfg, signal);
        }
 ...
 */
function patchFrames(page, cfg, cursor, raw, rawKb, originals, stealth, signal) {
...
async function frameHumanTypeTarget(frame, selector, sensitive) {
    return firstFrameLocator(frame, selector).evaluate((el, sensitive) => ({
        tag: el.tagName.toLowerCase(),
        type: el.getAttribute('type'),
        autocomplete: el.getAttribute('autocomplete'),
        contentEditable: el.isContentEditable,
...
function patchSingleFrame(frame, page, cfg, cursor, raw, rawKb, originals, stealth, signal) {
    ...
    frame._humanRestoreDescriptors = new Map(frameKeys.map(key => [key, Object.getOwnPropertyDescriptor(frame, key)]));
    const frames = patchedFrames.get(page) ?? new Set();
    frames.add(frame);

...
function selectedLocalBrowserKind() {
...
function runtimeReadinessHint(kind) {
    if (kind === 'slab')
        return 'Open SLAB and retry the browser command.';
    if (kind === 'cloak')
        return 'Make sure Chrome/Chromium is open and Cloak is enabled.';
    if (kind === 'chrome')
        return 'Make sure the configured Chrome runtime is open and reachable.';
    return 'Make sure the configured custom browser runtime is running and reachable.';
}
export function browserConnectErrorFromHealth(health, contextId, kind = selectedLocalBrowserKind()) {
    ...
    if (health.state === 'profile-disconnected') {
        ...
        return new BrowserConnectError(`Browser profile "${label}" is not connected`, `${kind === 'slab' ? 'Open SLAB and verify that Profile' : `Open the matching ${runtimeLabel(kind)} profile`} "${label}" is available, or choose another profile with webcmd profile use <name>.`, 'profile-disconnected');
    }
    if (health.state === 'no-runtime') {
        return new BrowserConnectError('Browser runtime is not ready', `${runtimeReadinessHint(kind)} Run \`webcmd doctor\` for local status.`, 'runtime-not-ready');
    }

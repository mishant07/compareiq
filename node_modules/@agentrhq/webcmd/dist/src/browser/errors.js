 ...
 *
 * Simplified — no more token/runtime/CDP classification.
 * The daemon architecture has a single failure mode: daemon not reachable or runtime not ready.
 */
import { BrowserConnectError } from '../errors.js';
import { DEFAULT_DAEMON_PORT } from '../constants.js';
/**
 * Runtime/daemon transient patterns — service worker restarts, attach races,
 * tab closure, daemon hiccups. These warrant a longer retry delay (~1500ms)
 * because the runtime needs time to recover.
 */
...
export function formatBrowserConnectError(kind, detail) {
    switch (kind) {
        case 'daemon-not-running':
            return new BrowserConnectError('Cannot connect to webcmd daemon.' + (detail ? `\n\n${detail}` : ''), `Run \`webcmd doctor\` to diagnose, or \`webcmd daemon restart\` to force a fresh daemon. Default port is ${DEFAULT_DAEMON_PORT}.`, kind);
        case 'runtime-not-ready':
        case 'extension-not-connected':
            return new BrowserConnectError('Browser runtime is not ready.' + (detail ? `\n\n${detail}` : ''), 'Open Chrome/Chromium with Cloak enabled and retry the browser command. Run `webcmd doctor` for local status.', 'runtime-not-ready');
        case 'command-failed':

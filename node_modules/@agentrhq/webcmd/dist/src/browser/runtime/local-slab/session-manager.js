...
export class SessionWindowConflictError extends CliError {
    constructor(pageId, sessionId, owner) {
        super('SESSION_WINDOW_CONFLICT', `Page ${pageId} is in a window owned by Session ${owner ?? 'unknown'}, not ${sessionId}.`, undefined, EXIT_CODES.TEMPFAIL);
    }
}
export class AdoptedTabForceRequiredError extends CliError {
    constructor(pageId) {
        super('ADOPTED_TAB_FORCE_REQUIRED', `Closing adopted human tab ${pageId} is destructive and requires --force.`, 'Detach the Session normally to leave the human window open.', EXIT_CODES.USAGE_ERROR);
    }
}
export class SlabAttachmentLostError extends Error {
    code = 'SLAB_ATTACHMENT_LOST';
    constructor() {
...
function daemonShuttingDownError() {
    return Object.assign(new Error('The browser daemon is shutting down.'), { code: 'DAEMON_SHUTTING_DOWN' });
}
function isBlankStartupUrl(url) {
    return url === 'about:blank' || url === 'chrome://newtab/' || url === 'chrome://new-tab-page/';
}
export class SlabSessionManager {
    opts;
    networkCapture = new SlabNetworkCapture();
    attachProfile;
    hasActiveHandoff;
    humanize;
    profiles = new Map();
    detachedSessions = new Map();
    profileLaunches = new Map();
    profileLifecycleQueues = new Map();
    profileActivities = new Map();
    pageCreationQueues = new Map();
    pageTargetIds = new WeakMap();
    pageTargetIdPromises = new WeakMap();
    pendingTargetPages = new WeakMap();
    pendingTargetCloseListeners = new WeakMap();
    targetPageWaiters = new WeakMap();
    sessionPageListeners = new WeakMap();
    shuttingDown = false;
    constructor(opts = {}) {
        this.opts = opts;
    ...
    async discoveredWindows(profileIdInput) {
        ...
        for (const page of pages)
            byWindow.set(page.windowId, [...(byWindow.get(page.windowId) ?? []), page]);
        const rows = [...byWindow.entries()].map(([windowId, tabs]) => ({
            rowKind: 'discovered',
            profileId,
    ...
    async runWithProfileActivity(profileIdInput, task) {
        const profileId = normalizeProfileId(profileIdInput);
    ...
    async getPage(input) {
        ...
        return this.withPageCreationLock(profileId, async () => {
            ...
            if (existing && freshPage && existing !== entry)
                await this.removeEntry(acquired.runtime, sessionRuntime, existing, true);
    ...
    async findPage(input) {
        ...
        if (!runtime || !sessionRuntime || !entry || pageIsClosed(entry.page))
            return null;
    ...
    async findPageById(pageId, opts) {
    ...
    async browserRunScope(input, page) {
        const profileId = normalizeProfileId(input.profileId);
        const sessionId = requireSessionId(input);
        this.assertSessionAttached(profileId, sessionId);
        const runtime = this.profiles.get(profileId);
        ...
        await Promise.all(this.openEntries(sessionRuntime).map(([, candidate]) => (this.assertOwnedWindow(runtime, sessionId, candidate))));
        const browser = runtime.context.browser();
        if (!browser)
            throw new Error('The selected browser context is not attached to a browser.');
        return {
            browser,
    ...
    async navigatePage(input, url, waitUntil) {
        return this.navigatePageAttempt(input, url, waitUntil, 0);
    }
    async newPageAttempt(input, attempt) {
        const profileId = normalizeProfileId(input.profileId);
        const session = requireSession(input.session);
        const sessionId = requireSessionId(input);
        const surface = normalizeSurface(input.surface);
    ...
    async navigatePageAttempt(input, url, waitUntil, attempt) {
        const profileId = normalizeProfileId(input.profileId);
        ...
        try {
            ...
            if (attempt !== 0 || !isClosedContextError(error))
                throw error;
            if (runtime?.context === lease.context)
                this.invalidateProfileRuntime(profileId, runtime);
            if (!pageIsClosed(lease.page))
    ...
    async foregroundSession(profileIdInput, sessionId) {
        const profileId = normalizeProfileId(profileIdInput);
        const runtime = this.profiles.get(profileId);
    ...
    async bindPage(input) {
        ...
        return this.withPageCreationLock(profileId, async () => {
            ...
            if (discovered && discovered.windowId !== windowId)
                return null;
            ...
            const restoreTransaction = () => {
                ...
                for (const [key, value] of sessionPagesSnapshot)
                    sessionRuntime.pages.set(key, value);
                sessionRuntime.windowIds.clear();
                for (const id of windowIdsSnapshot)
                    sessionRuntime.windowIds.add(id);
                runtime.windowOwners.clear();
                for (const [id, value] of windowOwnersSnapshot)
                    runtime.windowOwners.set(id, value);
                runtime.targetPages.clear();
                for (const [id, value] of targetPagesSnapshot)
                    runtime.targetPages.set(id, value);
                ...
                for (const [entry, snapshot] of entrySnapshots) {
                    ...
                    if (snapshot.hadIdleTimer)
                        this.refreshIdleTimer(runtime, sessionRuntime, entry.leaseKey, entry);
                }
                if (!existingSession && sessionPagesSnapshot.size === 0)
            ...
            try {
                for (const sibling of siblings) {
                    if (pageIsClosed(sibling))
                        throw new Error('SLAB bind target disappeared during window adoption.');
                ...
                for (const entry of registered) {
                    if (!entrySnapshots.has(entry))
                        await this.removeEntry(runtime, sessionRuntime, entry, false);
                }
                await Promise.all(newlyHumanized.map(sibling => disposeHumanizedPage(sibling)));
                await Promise.all(siblings.map(sibling => this.detachPageCdp(sibling)));
                restoreTransaction();
    ...
    async release(input) {
        ...
        const leaseKey = resolveLeaseKey(input);
        const surface = normalizeSurface(input.surface);
        const entries = this.openEntries(sessionRuntime).filter(([key, entry]) => (surface === 'adapter'
            ? key === leaseKey
            : entry.session === requireSession(input.session) && entry.surface === surface));
        await Promise.all(entries.map(([, entry]) => this.assertOwnedWindow(runtime, sessionId, entry)));
        for (const [, entry] of entries) {
            if (entry.siteSession === 'persistent' || entry.provenance === 'human-adopted')
                continue;
            await this.removeEntry(runtime, sessionRuntime, entry, true);
        }
    }
    hasSession(profileIdInput, sessionInput) {
        const profileId = normalizeProfileId(profileIdInput);
    ...
    async closeSession(profileIdInput, sessionInput) {
        const profileId = normalizeProfileId(profileIdInput);
        const session = requireSession(sessionInput);
        const runtime = this.profiles.get(profileId);
        if (!runtime)
     ...
     */
    invalidateIfClosedContext(profileId, context) {
        const runtime = this.profiles.get(profileId);
    ...
    async getProfileRuntime(profileId, windowMode) {
        return this.withProfileLifecycleLock(profileId, async () => {
            ...
            try {
                ...
                if (this.profileLaunches.get(profileId) === launch)
                    this.profileLaunches.delete(profileId);
            }
        });
    }
    async launchProfileRuntime(profileId, _windowMode) {
    ...
    invalidateProfileRuntime(profileId, runtime) {
        if (this.profiles.get(profileId) === runtime)
            this.profiles.delete(profileId);
        const detached = this.detachedSessions.get(profileId) ?? new Set();
        for (const sessionId of runtime.sessions.keys())
            detached.add(sessionId);
        if (detached.size > 0)
            this.detachedSessions.set(profileId, detached);
    ...
    async cleanupRuntime(runtime) {
        ...
        for (const { page, listener } of this.pendingTargetCloseListeners.get(runtime)?.values() ?? [])
    ...
    attachRuntimeLifecycle(profileId, runtime) {
        ...
        const onCdpEvent = runtime.cdp?.on;
        onCdpEvent?.call(runtime.cdp, 'Target.targetDestroyed', ({ targetId }) => {
            this.queueAnchorRepair(profileId, runtime, targetId);
        });
    }
    queueAnchorRepair(profileId, runtime, destroyedTargetId) {
        if (runtime.anchorTargetId !== destroyedTargetId)
            return;
        runtime.anchorTargetId = undefined;
        void this.withProfileLifecycleLock(profileId, async () => {
            if (this.shuttingDown || runtime.closing || this.profiles.get(profileId) !== runtime)
                return;
            if (runtime.anchorTargetId !== undefined)
                return;
            await this.repairAnchor(profileId, runtime);
        });
    }
    async repairAnchor(profileId, runtime) {
        if (!runtime.cdp)
            return;
        try {
    ...
    async ensureKeeper(runtime) {
        if (runtime.anchorTargetId || runtime.useParkingKeeper || !runtime.cdp)
            return;
        try {
            runtime.anchorTargetId = (await runtime.cdp.send('Target.createTarget', {
                url: 'about:blank',
    ...
    warnKeeperFallback(profileId, runtime, error) {
        runtime.useParkingKeeper = true;
        if (runtime.keeperWarningLogged)
            return;
    ...
    scheduleProfileIdle(profileId, runtime) {
        if (this.profiles.get(profileId) !== runtime || runtime.closing || runtime.idleTimer || runtime.handoffTimer)
            return;
        if (runtime.activeCommands > 0 || this.hasVisiblePages(runtime))
            return;
        ...
        runtime.idleTimer = setTimeout(() => {
            ...
            void this.withProfileLifecycleLock(profileId, async () => {
                if (this.profiles.get(profileId) !== runtime || runtime.closing)
                    return;
                if (runtime.activeCommands > 0 || this.hasVisiblePages(runtime))
                    return;
                if (this.hasActiveHandoff(profileId)) {
    ...
    scheduleHandoffWake(profileId, runtime) {
    ...
    cancelProfileIdle(runtime) {
        if (runtime.idleTimer)
            clearTimeout(runtime.idleTimer);
        if (runtime.handoffTimer)
            clearTimeout(runtime.handoffTimer);
        runtime.idleTimer = undefined;
        runtime.handoffTimer = undefined;
    }
    hasVisiblePages(runtime) {
        for (const session of runtime.sessions.values()) {
            if (this.openEntries(session).length > 0)
                return true;
        }
        return false;
    }
    ...
    async releaseRuntime(runtime, closePages, releaseNative = true) {
        if (runtime.releasePromise)
        ...
        runtime.releasePromise = (async () => {
            ...
            try {
                if (closePages) {
                    await Promise.all([...runtime.targetPages.values()].map(entry => (pageIsClosed(entry.page) || entry.provenance === 'human-adopted'
                        ? undefined
                ...
                if (releaseNative)
                    ...
                    runtime.attachment.closeTransport();
            }
            finally {
    ...
    async withProfileLifecycleLock(profileId, operation) {
        ...
        try {
            ...
            release();
            if (this.profileLifecycleQueues.get(profileId) === queue)
                this.profileLifecycleQueues.delete(profileId);
        }
    }
    assertRunning() {
        if (this.shuttingDown)
    ...
    assertSessionAttached(profileId, sessionId) {
        if (this.detachedSessions.get(profileId)?.has(sessionId))
            throw new SlabAttachmentLostError();
    }
    async withPageCreationLock(profileId, operation) {
        ...
        try {
            ...
            release();
            if (this.pageCreationQueues.get(profileId) === queue)
                this.pageCreationQueues.delete(profileId);
        }
    }
    getSessionRuntime(runtime, sessionId) {
    ...
    async createSessionPage(runtime, session, windowMode) {
        const openerEntry = this.openEntries(session)[0]?.[1];
        if (!openerEntry)
            return await this.claimSoleBlankStartupPage(runtime) ?? this.createWindowPage(runtime, windowMode);
    ...
    async claimSoleBlankStartupPage(runtime) {
        ...
        const unowned = (await this.discoverPages(runtime))
            .filter(candidate => runtime.windowOwners.get(candidate.windowId) === undefined);
        if (unowned.length !== 1 || !isBlankStartupUrl(unowned[0].url))
            return undefined;
        return await this.findPageByTargetId(runtime, unowned[0].targetId);
    }
    async waitForContextPageForSession(runtime, sessionId, openerWindowId, targetUrl, timeoutMs) {
        return new Promise((resolve) => {
            ...
            const onPage = (page) => { void tryPage(page); };
            const timer = setTimeout(() => done(null), timeoutMs);
            runtime.context.on('page', onPage);
            for (const page of this.pendingTargetPages.get(runtime)?.values() ?? [])
                void tryPage(page);
        });
    }
    async acquireSessionPage(profileId, sessionId, windowMode, attempt = 0) {
        this.assertSessionAttached(profileId, sessionId);
    ...
    async createWindowPage(runtime, windowMode, newWindow = true) {
    ...
    async waitForTargetPage(runtime, targetId) {
    ...
    async findPageByTargetId(runtime, targetId) {
        const pending = this.pendingTargetPages.get(runtime)?.get(targetId);
        if (pending && !pageIsClosed(pending))
            return pending;
    ...
    windowIdentity(runtime, windowId) {
        return `slab-window-${runtime.generation}-${windowId}`;
    }
    async discoverPages(runtime) {
        if (!runtime.cdp)
            return [];
        const { targetInfos = [] } = await runtime.cdp.send('Target.getTargets');
        ...
        for (const [pageId, page] of runtime.discoveredPages) {
            if (!live.has(page.targetId))
                runtime.discoveredPages.delete(pageId);
        }
        const result = [];
        for (const target of targetInfos) {
            if (target.type !== 'page' || target.targetId === runtime.anchorTargetId)
                continue;
            if (target.url?.startsWith('devtools://') || target.url?.startsWith('chrome-extension://'))
                continue;
    ...
    async handleContextPage(runtime, page) {
        ...
        try {
            await this.withPageCreationLock(runtime.profileId, async () => {
                if (runtime.disposed || this.profiles.get(runtime.profileId) !== runtime)
                    return;
    ...
    async registerOwnedPage(runtime, session, page, input) {
        ...
        try {
            ...
            if (!entry) {
                ...
                if (entry.sessionId) {
                    ...
                    if (session?.pages.get(entry.leaseKey) === entry)
                        session.pages.delete(entry.leaseKey);
                }
            ...
            if (behavior)
                this.humanize(page, behavior.traits);
            this.cancelProfileIdle(runtime);
            this.refreshIdleTimer(runtime, session, entry.leaseKey, entry);
            if (!wasOwned)
                for (const listener of this.sessionPageListeners.get(session) ?? [])
                    listener(page);
            await this.closeParkingPage(runtime);
            return entry;
        }
            ...
            if (entry?.idleTimer)
                clearTimeout(entry.idleTimer);
            session.pages.clear();
            for (const [key, value] of priorSessionPages)
                session.pages.set(key, value);
            session.windowIds.clear();
            for (const id of priorWindowIds)
                session.windowIds.add(id);
            if (priorOwner === undefined)
                runtime.windowOwners.delete(windowId);
            else
                runtime.windowOwners.set(windowId, priorOwner);
            if (priorEntry && entry) {
                Object.assign(entry, priorEntry);
                entry.idleTimer = undefined;
                if (priorHadIdleTimer)
                    this.refreshIdleTimer(runtime, session, entry.leaseKey, entry);
            }
            else if (entry) {
    ...
    async ensureBehavior(runtime) {
        if (runtime.behavior)
            return runtime.behavior;
        try {
            runtime.behavior = await loadOrCreateBehaviorProfile(runtime.profileId, { baseDir: this.opts.baseDir });
            if (runtime.behavior.warning)
                log.warn(runtime.behavior.warning);
            return runtime.behavior;
        }
        catch (error) {
            throw new Error(`Unable to load SLAB behavioral state for profile "${runtime.profileId}": ${errorMessage(error)}`);
        }
    }
    attachPageLifecycle(runtime, entry) {
        const closeListener = () => {
            if (runtime.parkingPage === entry.page)
                runtime.parkingPage = undefined;
            const session = entry.sessionId ? runtime.sessions.get(entry.sessionId) : undefined;
            if (session)
                void this.removeEntry(runtime, session, entry, false);
            else {
        ...
        const correlation = (async () => {
            ...
            try {
                ...
                if (!targetInfo?.targetId)
                    throw new Error('SLAB page target identity is unavailable.');
    ...
    async targetIdForPageInWindow(runtime, page, expectedWindowId) {
        const cached = this.pageTargetIds.get(page);
        if (cached) {
    ...
    async openerTargetIdForTarget(runtime, targetId) {
        if (!runtime.cdp)
            return undefined;
        const { targetInfo } = await runtime.cdp.send('Target.getTargetInfo', { targetId });
        return targetInfo.openerId;
    }
    async windowIdForTarget(runtime, targetId, page) {
        const entry = runtime.targetPages.get(targetId);
        const cdp = runtime.cdp;
        if (!cdp)
            throw new Error('SLAB page has no CDP session.');
        const { windowId } = await cdp.send('Browser.getWindowForTarget', { targetId });
        return windowId;
    }
    async assertOwnedWindow(runtime, sessionId, entry) {
    ...
    sessionEntries(runtime, input) {
        const session = requireSession(input.session);
        const surface = normalizeSurface(input.surface);
        return this.openEntries(runtime).filter(([, entry]) => entry.session === session && entry.surface === surface);
    }
    selectEntry(runtime, entry) {
        runtime.selectedPageId = entry.pageId;
    }
    clearSelectedPage(runtime, entry) {
        if (runtime.selectedPageId === entry.pageId)
            runtime.selectedPageId = undefined;
    }
    refreshIdleTimer(runtime, session, leaseKey, entry) {
        this.clearIdleTimer(entry);
        if (!entry.idleTimeout || entry.idleTimeout <= 0 || entry.siteSession === 'persistent' || entry.provenance === 'human-adopted')
            return;
    ...
    async expireLease(runtime, session, leaseKey, entry) {
        if (session.pages.get(leaseKey) !== entry)
            return;
        runtime.lastSeenAt = Date.now();
        if (entry.siteSession !== 'persistent')
            await this.removeEntry(runtime, session, entry, true);
    }
    async removeEntry(runtime, session, entry, close, force = false) {
        ...
        const windowStillOwned = [...runtime.targetPages.values()].some(candidate => (candidate.sessionId === session.id && candidate.windowId === entry.windowId && !pageIsClosed(candidate.page)));
        if (!windowStillOwned) {
    ...
    async closeParkingPage(runtime) {
        const parkingPage = runtime.parkingPage;
        if (!parkingPage)
            return;
        runtime.parkingPage = undefined;
        if (!pageIsClosed(parkingPage))
            await parkingPage.close().catch(() => { });
        await this.detachPageCdp(parkingPage);
    }
    detachPageCdp(_page) {
        return Promise.resolve();
    }
    rememberPendingTargetPage(runtime, targetId, page) {
        this.clearPendingTargetPage(runtime, targetId);
    ...
    clearPendingTargetPage(runtime, targetId, expectedPage) {
        const pending = this.pendingTargetPages.get(runtime);
        if (expectedPage && pending?.get(targetId) !== expectedPage)
            return;
        pending?.delete(targetId);
        const closeListeners = this.pendingTargetCloseListeners.get(runtime);
        const closeListener = closeListeners?.get(targetId);
        if (closeListener)
            closeListener.page.off('close', closeListener.listener);

...
import { SlabAttachmentLostError } from './session-manager.js';
...
export function resolveSlabCommandProfileId(manager, command) {
    ...
    if (active.length === 1)
        return active[0];
    if (active.length > 1) {
        throw new SlabActionError('profile_required', `Default SLAB profile "${preferred}" is not active and multiple profiles are running; choose one with --profile.`, undefined, 'Run webcmd profile list, then update the default with webcmd profile use <name> or pass --profile <name>.');
    }
    return preferred;
}
...
async function resolveExistingLease(manager, command) {
    ...
    if (existing)
        return existing;
    throw new SlabActionError('session_not_found', `Browser session not found: ${command.session ?? ''}`, undefined, 'Start the session with browser run or navigate before requesting a snapshot.');
}
function execTarget(page, frameIndex, pageId) {
    ...
    if (!frame)
        throw new SlabActionError('frame_not_found', `Frame not found: ${frameIndex}`, pageId);
    return frame;
}
...
export async function dispatchSlabAction(manager, command, signal) {
    try {

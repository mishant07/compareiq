...
let cachedCloakBrowserVersion = UNRESOLVED;
/**
...
export class CloakSessionManager {
    ...
    syncExportsInFlight = new Set();
    chromeSyncExportTimers = new Map();
    profiles = new Map();
    ...
    async bindPage(input) {
        ...
        if (!match && input.index !== undefined) {
            ...
            for (const candidate of runtime.context.pages()) {
                if (pageIsClosed(candidate) || candidate === runtime.parkingPage)
                    continue;
                if (await this.targetIdForPage(runtime, candidate) === runtime.anchorTargetId)
                    continue;
    ...
    async shutdown() {
        ...
        for (const timer of this.chromeSyncExportTimers.values())
            clearTimeout(timer);
    ...
    async closeRuntime(runtime, recoverOnTimeout) {
        this.cancelProfileIdle(runtime);
        for (const entry of runtime.targetPages.values())
    ...
    scheduleChromeSyncExport(profileId, runtime) {
        if (this.opts.runtimeKind !== 'chrome' || this.opts.syncToChrome !== true)
            return;
        this.cancelChromeSyncExport(profileId);
    ...
    cancelChromeSyncExport(profileId) {
    ...
    scheduleChromeExport(profileId, runtime) {
        if (this.opts.runtimeKind !== 'chrome' || this.opts.syncToChrome !== true)
            return;
        if (this.syncExportsInFlight.has(profileId))
            return;
    ...
    async exportProfileToChrome(profileId, runtime) {
        const nativeUserDataDir = chromeUserDataDir({ platform: this.platform });
        if (!nativeUserDataDir)
            return;
        let created;
        try {
    ...
    async createSessionPage(runtime, session, windowMode) {
        ...
        if (!openerEntry)
            return await this.findReusableLaunchPage(runtime, session.id) ?? this.createWindowPage(runtime, windowMode);
    ...
    async findReusableLaunchPage(runtime, sessionId) {
        for (const page of runtime.context.pages()) {
            if (pageIsClosed(page) || page === runtime.parkingPage || page.url() !== 'about:blank')
                continue;
            const targetId = await this.targetIdForPage(runtime, page).catch(() => undefined);
            if (!targetId || targetId === runtime.anchorTargetId || runtime.targetPages.has(targetId))
                continue;
    ...
    async windowIdForTarget(runtime, targetId, page) {
        ...
        if (!cdp)
            throw new Error('Cloak page has no CDP session.');
    ...
    async assertBindableWindow(runtime, session, entry) {
        if (entry.sessionId) {
    ...
    async removeEntry(runtime, session, entry, close) {
...
async function recoverLockedChromeProfile(executablePath, userDataDir, platform) {
    ...
    for (const pid of initial)
        await terminateChromeProcessTree(pid, platform, false);
    const deadline = Date.now() + 2500;
    while (Date.now() < deadline) {
        await new Promise(resolve => setTimeout(resolve, 100));
        if ((await findExactChromeProcesses(identity, platform)).length === 0)
            return true;
    }
...
async function waitForProfileProcessesToExit(userDataDir, timeoutMs) {
    ...
    while (Date.now() < deadline) {
        await new Promise((resolve) => setTimeout(resolve, 100));
        if ((await findExactCloakProfileProcesses(userDataDir)).length === 0)
            return true;
    }

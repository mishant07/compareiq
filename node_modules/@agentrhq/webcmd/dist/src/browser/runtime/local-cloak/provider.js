...
export class LocalCloakRuntimeProvider {
    ...
    constructor(opts = {}) {
        ...
        this.manager = new CloakSessionManager({
            ...
            hasActiveHandoff: profileId => this.sessions.list(profileId, 100).some(session => (Boolean(session.handoff) && Date.parse(session.handoff.expiresAt) > Date.now())),
        });
    }
    async status(opts = {}) {
    ...
    async closeSession(command) {
        ...
        if (command.force && command.discard === true && record.kind === 'explicit' && !record.handoff)
            this.sessions.remove(record.profileId, record.id);
        else if (command.force && record.handoff)
            this.sessions.clearHandoff(record.profileId, record.id);
        else
            this.sessions.touch(record.profileId, record.id);
        return { closed: closedCount > 0, alreadyIdle: closedCount === 0, session: record.id };
    }
    async dispatch(rawCommand, signal) {
        ...
        const command = rawCommand.windowMode
            ? rawCommand
        ...
        try {
            ...
            return await this.manager.runWithProfileActivity(this.resolveProfileId(command), () => dispatchCloakAction(this.manager, command, signal));
        }
        finally {
            release();

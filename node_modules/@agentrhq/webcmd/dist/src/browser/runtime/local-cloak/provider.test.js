...
function fakePage(url, initialViewport = { width: 1280, height: 720 }, opener = null) {
    let closed = false;
    ...
    const page = {
        ...
        setViewportSize: vi.fn(async (size) => {
            viewportSize = size;
        }),
        locator: vi.fn(),
        waitForEvent: vi.fn(),
...
function makeProviderWithFakePage(initialViewport = { width: 1280, height: 720 }) {
    ...
    const context = {
        ...
        waitForEvent: vi.fn((event) => new Promise((resolve) => context.on(event, resolve))),
        ...
        newCDPSession: vi.fn(async (target) => {
            const pageSession = {
                send: vi.fn(async (command, params) => {
                    if (params === undefined)
                        cdpSession.send(command);
                    else
                        cdpSession.send(command, params);
    ...
    cdpSession.send.mockImplementation(async (command, params) => {
...
describe('LocalCloakRuntimeProvider', () => {
    ...
    it.each([
        ...
        { label: 'asks for foreground', windowMode: 'foreground', background: false, focus: true },
        ...
        const windowTargets = cdpSession.send.mock.calls
            .filter(([command, params]) => command === 'Target.createTarget' && !params?.hidden)
            .map(([, params]) => params);
        expect(windowTargets.length).toBeGreaterThan(0);
        for (const params of windowTargets)
            expect(params).toMatchObject({ background, focus });
    });
    ...
    it('does not execute a queued command after its daemon deadline expires', async () => {
        const { provider, page } = makeProviderWithFakePage();
        let releaseFirst;
        page.goto.mockImplementationOnce(() => new Promise((resolve) => {
    ...
    it('preserves structured browser-run error details', async () => {
        const { provider } = makeProviderWithFakePage();
        runBrowserProgram.mockRejectedValue(new BrowserRunError('BROWSER_RUN_TIMEOUT', 'Timed out', undefined, {
    ...
    it('partitions the local queue by adapter site only for adapter-default Sessions', () => {
        const { provider } = makeProviderWithFakePage();
        const queueKey = provider.commandQueueKey.bind(provider);
        expect(queueKey({
            id: 'github',
            action: 'exec',
            surface: 'adapter',
    ...
    it('does not adopt a sibling Session page created during browser-run', async () => {
        ...
        runBrowserProgram.mockImplementationOnce(async (input) => {
            ...
            expect(input.pages().map((candidate) => candidate.url()))

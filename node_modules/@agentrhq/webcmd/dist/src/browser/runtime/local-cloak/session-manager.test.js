...
function fakeContext() {
    ...
    let windowCounter = 0;
    let context;
    const emitPageEvent = (page, event, ...args) => {
        for (const listener of pageListeners.get(page)?.get(event) ?? [])
            listener(...args);
    };
    const fakePage = (opener, windowId = ++windowCounter, initialUrl = 'https://example.com/') => {
        let closed = false;
    ...
    const cdp = {
        send: vi.fn(async (command, params) => {
            if (command === 'Target.createTarget') {
                let backgroundPage;
                if (params?.hidden) {
                    backgroundPage = fakePage();
                    allPages.push(backgroundPage);
                }
                else if (params?.newWindow === false) {
                    let opener;
                    for (let index = allPages.length - 1; index >= 0; index -= 1) {
                        if (!allPages[index].isClosed()) {
                            opener = allPages[index];
                            break;
                        }
                    }
                    backgroundPage = fakePage(undefined, opener ? windowIds.get(targetIds.get(opener)) : undefined);
                    allPages.push(backgroundPage);
                }
                else {
            ...
            if (command === 'Browser.getWindowForTarget')
                return { windowId: windowIds.get(params?.targetId ?? '') };
    ...
    return {
        context: context = {
            ...
            newCDPSession: vi.fn(async (target) => ({
                send: vi.fn(async (command, params) => {
                    if (command === 'Target.getTargetInfo')
                        return { targetInfo: { targetId: targetIds.get(target) } };
                    if (command === 'Browser.getWindowForTarget')
                        return { windowId: windowIds.get(params?.targetId ?? '') };
                    return {};
                }),
        ...
        backgroundPages,
        cdp,
        targetIdFor: (target) => targetIds.get(target),
        windowIdFor: (target) => windowIds.get(targetIds.get(target) ?? ''),
        moveToWindow: (target, windowId) => windowIds.set(targetIds.get(target), windowId),
        emitPage: (target) => emit('page', target),
        emitPageEvent,
        emitCdp: (event, payload) => {
            for (const listener of cdpListeners.get(event) ?? [])
                listener(payload);
        },
        pageListenerCount: (target, event) => pageListeners.get(target)?.get(event)?.size ?? 0,
        makePage: fakePage,
    };
}
...
describe('CloakSessionManager', () => {
    ...
    it('passes the configured executable and namespace through to Cloak', async () => {
        ...
        expect(launchPersistentContext).toHaveBeenCalledWith(expect.objectContaining({
            userDataDir: path.join('/tmp/webcmd-test', 'custom-chromium-12345678', 'profiles', 'default'),
    ...
    it('correlates created targets and isolates Sessions into owned windows', async () => {
        ...
        expect(launched.cdp.send.mock.calls.filter(([method, params]) => method === 'Target.createTarget' && !params?.hidden))
            .toHaveLength(2);
    ...
    it('matches Target.createTarget by target id instead of adopting the next context page', async () => {
        ...
        launched.cdp.send.mockImplementationOnce(async (method, params) => {
    ...
    it('creates later Session pages with noopener and adopts the context page in the same window', async () => {
        ...
        const key = { profileId: 'default', session: 'session_a', sessionId: 'session_a', surface: 'browser' };
        ...
        expect(launched.cdp.send.mock.calls.filter(([method, params]) => (method === 'Target.createTarget' && !params?.hidden))).toHaveLength(1);
    ...
    it('falls back to another owned window when Chromium does not create the requested tab', async () => {
        ...
        const key = { profileId: 'default', session: 'session_a', sessionId: 'session_a', surface: 'browser' };
        const first = await manager.getPage(key);
        vi.mocked(first.page.evaluate).mockResolvedValueOnce(null);
        const second = await manager.newPage(key);
    ...
    it('ignores an unmarked opener-less page when waiting for a Session tab', async () => {
        ...
        expect(launched.cdp.send.mock.calls.filter(([method, params]) => (method === 'Target.createTarget' && !params?.hidden))).toHaveLength(2);
        expect((await manager.listPages(key)).map(tab => tab.sessionId)).toEqual(['session_a', 'session_a']);
    });
    it('keeps a site popup owned while noopener tab creation uses another page', async () => {
        ...
        vi.mocked(first.page.evaluate).mockImplementationOnce(async () => {
            queueMicrotask(() => {
                launched.emitPageEvent(first.page, 'popup', popup);
    ...
    it('creates a later page in its Session window when another Session was used last', async () => {
        ...
        const firstKey = { profileId: 'default', session: 'session_a', sessionId: 'session_a', surface: 'browser' };
        const first = await manager.getPage(firstKey);
        await manager.getPage({ profileId: 'default', session: 'session_b', sessionId: 'session_b', surface: 'browser' });
        const second = await manager.newPage(firstKey);
        expect(launched.windowIdFor(second.page)).toBe(launched.windowIdFor(first.page));
    });
    it('times out target correlation and releases the profile creation lock', async () => {
        ...
        const launched = fakeContext();
        const send = launched.cdp.send.getMockImplementation();
        launched.cdp.send.mockImplementation(async (method, params) => (method === 'Target.createTarget' && !params?.hidden
    ...
    it('rejects every operation after a Session page moves into another owned window', async () => {
        ...
        const a = { profileId: 'default', session: 'session_a', sessionId: 'session_a', surface: 'browser' };
        const b = { profileId: 'default', session: 'session_b', sessionId: 'session_b', surface: 'browser' };
    ...
    it('closes a Session when its page disappears during the ownership check', async () => {
        ...
        const send = launched.cdp.send.getMockImplementation();
        launched.cdp.send.mockImplementation(async (method, params) => {
    ...
    it('invalidates a reused getPage() lease when the CDP liveness probe finds a dead context (webcmd#314)', async () => {
        ...
        first.cdp.send.mockImplementation(async (command) => {
            if (command === 'Browser.getWindowForTarget') {
        ...
        expect(firstLease.page.close).toHaveBeenCalled();
        expect(launchPersistentContext).toHaveBeenCalledTimes(2);
    });
    ...
    it('retries initial navigation once after a closed-context failure', async () => {
    ...
    it('closes ephemeral adapter sessions when released', async () => {
        ...
        const key = { profileId: 'default', session: 'session_default', sessionId: 'session_default', surface: 'adapter', siteSession: 'ephemeral', adapterSite: 'github', runId: 'run_a' };
        const lease = await manager.getPage(key);
    ...
    it('releases only the owning ephemeral adapter site and run', async () => {
        ...
        const base = { profileId: 'default', session: 'session_default', sessionId: 'session_default', surface: 'adapter', siteSession: 'ephemeral' };
    ...
    it('keeps persistent adapter pages tracked when release is requested', async () => {
        ...
        const key = { profileId: 'default', session: 'session_default', sessionId: 'session_default', surface: 'adapter', siteSession: 'persistent', adapterSite: 'github', runId: 'run_a' };
        const lease = await manager.getPage(key);
        await manager.release(key);
    ...
    it('retries action navigation once after a closed-context failure', async () => {
        const first = fakeContext();
    ...
    it('does not invalidate a replacement runtime when stale navigation fails', async () => {
        ...
        first.page.goto.mockImplementationOnce(() => new Promise((_, reject) => { rejectNavigation = reject; }));
        const replacement = fakeContext();
        replacement.context.newPage.mockResolvedValue(replacement.page);
        const launchPersistentContext = vi.fn()
            .mockResolvedValueOnce(first.context)
            .mockResolvedValueOnce(replacement.context);
        const manager = new CloakSessionManager({ baseDir: '/tmp/webcmd-test', launchPersistentContext });
        const key = { profileId: 'default', session: 'work', sessionId: 'session_a', surface: 'browser' };
        await manager.getPage(key);
        const navigation = manager.navigatePage(key, 'https://example.com/', 'load');
        await vi.waitFor(() => expect(first.page.goto).toHaveBeenCalledTimes(1));
        first.context.emit('close');
        const replacementLease = await manager.getPage(key);
        rejectNavigation(new Error('Target page, context or browser has been closed'));
        await expect(navigation).resolves.toMatchObject({ context: replacement.context });
        expect(replacementLease.context).toBe(replacement.context);
        expect(launchPersistentContext).toHaveBeenCalledTimes(2);
    });
    ...
    it('falls back to a parking keeper when macOS rejects the hidden target', async () => {
        const launched = fakeContext();
        const send = launched.cdp.send.getMockImplementation();
        launched.cdp.send.mockImplementation((method, params) => (method === 'Target.createTarget' && params?.hidden
    ...
    it.each(['linux', 'win32'])('reuses a warm %s profile and replaces its parking page on the next Session', async (platform) => {
        vi.useFakeTimers();
        const launched = fakeContext();
        const launchPersistentContext = vi.fn().mockResolvedValue(launched.context);
        const manager = new CloakSessionManager({
        ...
        await manager.closeSession('work', 'session_a');
        await vi.advanceTimersByTimeAsync(59_999);
        const second = await manager.runWithProfileActivity('work', () => (manager.getPage({ profileId: 'work', session: 'session_b', surface: 'browser' })));
    ...
    it('repairs one anchor for duplicate destruction and page-close notifications', async () => {
        ...
        await anchor.close();
        await vi.waitFor(() => expect(launched.cdp.send.mock.calls.filter(([, params]) => (params?.hidden))).toHaveLength(2));
        await Promise.resolve();
        expect(launched.cdp.send.mock.calls.filter(([, params]) => (params?.hidden))).toHaveLength(2);
    });
...
describe('syncToChrome export on profile close', () => {
    ...
    function chromeManager(overrides = {}) {
        const launched = fakeContext();
    ...
    const key = { profileId: 'webcmd-work', session: 's1', surface: 'browser' };

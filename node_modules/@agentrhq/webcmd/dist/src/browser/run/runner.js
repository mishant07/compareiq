...
function isDownloadWait(text) {
    return /waiting for event ["']download["']/i.test(text)
        || /waitForEvent\(\s*['"]download['"]\s*\)/.test(text);
}
function isPopupOrNewTabWait(text) {
    return /waiting for event ["'](popup|page)["']/i.test(text)
        || /waitForEvent\(\s*['"](?:popup|page)['"]\s*\)/.test(text);
}
function timeoutKind(message, source) {
    if (isPopupOrNewTabWait(message) || (source !== undefined && isPopupOrNewTabWait(source)))
        return 'popup';
    if (isDownloadWait(message) || (source !== undefined && isDownloadWait(source)))
        return 'download';
    return undefined;
}
function timeoutRunError(message, source) {
    const kind = timeoutKind(message, source);
    if (kind === 'popup') {
        return new BrowserRunError('BROWSER_RUN_TIMEOUT', isPopupOrNewTabWait(message) ? message : 'Browser-run timed out waiting for a popup or new tab.', POPUP_WAIT_TIMEOUT_HINT);
    }
    if (kind === 'download') {
        return new BrowserRunError('BROWSER_RUN_TIMEOUT', isDownloadWait(message) ? message : 'Browser-run timed out waiting for a download.', DOWNLOAD_WAIT_TIMEOUT_HINT);
    }
...
function normalizeExecutionError(error) {
    ...
    if (/Storage is disabled inside data:/i.test(message) || /Access is denied for this document/i.test(message)) {
        return new BrowserRunError('BROWSER_RUN_INVALID_INPUT', 'localStorage is disabled on data: and about:blank documents.', 'Navigate to an http(s) URL, or use page.evaluate memory. data: pages cannot use localStorage.');
    }
    if (/\b(require|module|process|__dirname|__filename|fs)\b.*\b(is not defined|unavailable)|Cannot find module ['"]fs['"]/i.test(message)) {
        return new BrowserRunError('BROWSER_RUN_API_UNSUPPORTED', 'Node require/fs are unavailable in the browser-run sandbox.', NODE_SANDBOX_HINT);
    }
...
export async function runBrowserProgram(input, source, options = {}) {
    ...
    try {
        ...
        for (const message of pendingDelivery)
            host.deliverTransport(message);
    ...
    `, { filename: 'browser-run-bootstrap.js' });
        ...
        const timeoutError = () => timeoutRunError(`Browser-run execution exceeded ${timeoutMs}ms.`, source);

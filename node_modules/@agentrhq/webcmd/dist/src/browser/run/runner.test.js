...
const playwrightServer = createRequire(import.meta.url)('playwright-core/lib/coreBundle');
let browser;
let context;
let page;
const describeWithChromium = fs.existsSync(chromium.executablePath()) ? describe : describe.skip;
function sessionScope(pages = () => context.pages()) {
    return {
        browser,
        context,
        page,
...
function run(source, options = {}, input = {}) {
...
async function runError(source) {
    try {
        await run(source);
    }
    catch (cause) {
...
describeWithChromium('runBrowserProgram', () => {
  ...
  `);
    ...
    it('omits a structural diff when critical snapshot content would be lost', async () => {
        const controls = Array.from({ length: 20 }, (_, index) => `<input aria-label="Critical ${index + 1}" aria-invalid="true">`).join('');
        const output = await run(`
    ...
    it('does not execute the program when the pre-snapshot fails', async () => {
        context.newCDPSession = (() => Promise.reject(new Error('pre snapshot failed')));
        await expect(run(`
      await page.getByRole('button', { name: 'Save' }).click();
      return null;
    ...
    it('counts the pre-snapshot against the command deadline', async () => {
        page.evaluate = (() => new Promise(() => undefined));
        await expect(run(`
      await page.getByRole('button', { name: 'Save' }).click();
      return null;
    ...
    it('keeps program success and warns when the post-snapshot fails', async () => {
        ...
        let calls = 0;
        context.newCDPSession = ((...args) => {
            calls += 1;
            if (calls === 2)
                return Promise.reject(new Error('post snapshot failed'));
    ...
    it('resolves waitForEvent("popup") into a readable Page before the outer run timeout', async () => {
        ...
        const output = await runBrowserProgram({
            ...
            pageId: 'page-1',
            onPage(listener) {
                const registeredListener = (popup) => {
                    if (!registered.includes(popup))
    ...
    it('initializes against a pre-launched persistent context without registering it twice', async () => {
        ...
        try {
            ...
            if (!persistentBrowser)
                throw new Error('persistent browser missing');
    ...
    it.each(['popup', 'download'])('honors waitForEvent("%s") timeout when no event fires', async (event) => {
    ...
    it.each(['writeArtifact', '__webcmdWriteArtifact'])('returns a redeemable receipt from %s', async (fn) => {
        ...
        `, {}, { artifactSink: new LocalBrowserRunArtifactSink({ baseDir }) });
            ...
            expect(fs.readFileSync(path.join(baseDir, receipt.artifactId, 'nested/report.csv'), 'utf8')).toBe('id,name\n1,caf\u00e9\n');
        }
        finally {
    ...
    it.each(['/tmp/shot.png', '../shot.png', 'nested/../../shot.png'])('rejects non-logical screenshot path %s', async (artifactPath) => {
        await expect(run(`
    ...
    it('returns a wall timeout and disposes QuickJS without waiting for protocol cleanup', async () => {
        ...
        let disposed = false;
        root.stopPendingOperations = () => new Promise(() => undefined);
    ...
    it('redacts page metadata and execution errors', async () => {
        ...
        expect(output.page.url).toBe('https://[REDACTED]@example.test/path?token=[REDACTED]');
        await expect(run(`
...
describe('unsupportedApiMessage', () => {
    ...
    it('keeps the generic text for an unmapped API', () => {
        expect(unsupportedApiMessage('Browser.killForTests')).toBe('Browser.killForTests is unavailable in browser run.');
    });
});

...
describe('waitForCaptureJs', () => {
    it('returns a non-empty string', () => {
        ...
        expect(code).toContain('__webcmd_xhr');
        expect(code).toContain('resolve');
        expect(code).toContain('reject');
    });
    it('resolves "captured" when __webcmd_xhr is populated before deadline', async () => {
        const g = globalThis;
        const captured = [];
    ...
    it('rejects when __webcmd_xhr stays empty past deadline', async () => {
        const g = globalThis;
        g.__webcmd_xhr = [];
        g.window = g;
    ...
    it('uses monotonic elapsed time when the wall clock jumps forward', async () => {
        ...
        try {
            const promise = eval(waitForCaptureJs(200));

...
const { mockBrowserConnect, mockBrowserClose, mockBindTab, mockListExistingBrowserTabs, mockReleaseSiteSessionLease, mockSendCommand, mockExecFileSync, browserState, } = vi.hoisted(() => ({
...
afterEach(() => {
    ...
    if (realConfigDir === undefined)
        ...
        process.env.WEBCMD_CONFIG_DIR = realConfigDir;
...
describe('plugin update reconciliation reporting', () => {
    ...
    it('reports every monorepo plugin refreshed by a named update', async () => {
        ...
        const update = vi.spyOn(pluginModule, 'updatePlugin').mockReturnValue(['alpha', 'beta']);
...
describe('createProgram root help descriptions', () => {
    ...
    it('binds update and convention-audit actions to their own commands', () => {
        ...
        expect(update._actionHandler).toEqual(expect.any(Function));
        expect(audit._actionHandler).toEqual(expect.any(Function));
        expect(update._actionHandler)
            .not.toBe(audit._actionHandler);
    });
    ...
    it('renders root structured help with built-ins and site adapter names', () => {
        ...
        try {
            ...
            expect(data.commands.map((cmd) => cmd.name)).not.toContain('youtube');
        }
        finally {
            process.argv = argv;
            registry.clear();
    ...
    it('separates command args from common options in structured help', () => {
        ...
        try {
            ...
            expect(data.common_options.map((option) => option.name)).toEqual(['format', 'json', 'trace', 'verbose', 'help']);
    ...
    it('points the retired browser fork subcommand at adapter override', () => {
        ...
        const run = (argv) => {
            ...
            try {
                ...
                handleProgramParseError(err, { write: (value) => { output += value; return true; } });
            }
    ...
    it('renders browser namespace structured help from Commander metadata', () => {
        ...
        try {
            ...
            expect(data.commands.map((cmd) => cmd.name)).toEqual(['bind', 'close', 'init', 'run', 'snapshot', 'tabs', 'verify']);
            ...
            const bind = data.commands.find((cmd) => cmd.name === 'bind');
            expect(bind).toMatchObject({
                command: 'webcmd browser bind',
                usage: 'webcmd browser bind [options]',
                positionals: [],
            });
            expect(bind.command_options.map((option) => option.name)).toEqual(['page', 'targetId', 'verbose', 'format', 'json']);
    ...
    it('renders plugin namespace structured help with positional + option leaves', () => {
        ...
        try {
            ...
            expect(update.command_options.map((option) => option.name)).toEqual(['all', 'force', 'format', 'json']);
        }
        finally {
            process.argv = argv;
        }
    ...
    it('renders adapter namespace structured help preserving original description after applyRootSubcommandSummaries', () => {
        ...
        try {
            ...
            expect(data.commands.map((cmd) => cmd.name)).toEqual(['override', 'path', 'reset', 'source get', 'source put', 'status']);
            const reset = data.commands.find((cmd) => cmd.name === 'reset');
            expect(reset).toMatchObject({
                usage: 'webcmd adapter reset [site] [options]',
                positionals: [{ name: 'site' }],
            });
            expect(reset.command_options.map((option) => option.name)).toEqual(['all', 'format', 'json']);
        }
        finally {
            process.argv = argv;
        }
    ...
    it('renders profile namespace structured help including required positionals', () => {
        ...
        try {
            ...
            expect(data.commands.map((cmd) => cmd.name)).toEqual(['create', 'list', 'rename', 'use']);
...
describe('browser verify', () => {
    ...
    describe('structured output', () => {
         ...
         */
        const runVerify = async (slug, adapterOutput, argv, seedFixture) => {
            const originalHome = process.env.HOME;
            const originalUserProfile = process.env.USERPROFILE;
        ...
        it('reports row-shape violations as structured failures', async () => {
            const stdout = await runVerify('json-shape', JSON.stringify([{ title: 'ok', author: { user_id: 'u1' } }]), ['--no-fixture', '-f', 'json']);
            expect(process.exitCode).toBe(1);
            const report = JSON.parse(stdout);
            expect(report.ok).toBe(false);
            expect(report.rowCount).toBe(1);
            expect(report.shapeFailures).toEqual(expect.arrayContaining([expect.objectContaining({ rule: 'shapeNestedId' })]));
            expect(stdout).not.toContain('violates row shape conventions');
        });
        it('reports fixture mismatches as structured failures', async () => {
            const stdout = await runVerify('json-mismatch', JSON.stringify([{ title: 'actual' }]), ['-f', 'json'], { expect: { columns: ['title'], rowCount: { min: 5 } } });
            expect(process.exitCode).toBe(1);
            const report = JSON.parse(stdout);
            expect(report.ok).toBe(false);
            expect(report.fixture).toMatchObject({ exists: true });
            expect(report.matchFailures).toEqual(expect.arrayContaining([expect.objectContaining({ rule: 'rowCount' })]));
        });
...
describe('structured output for data-returning built-ins', () => {
    ...
    const daemonStatusResponse = (overrides = {}) => ({
        ok: true,
        json: async () => ({
            ok: true,
            pid: 123,
...
describe('browser raw session commands', () => {
    ...
    describe('raw browser verbose flag', () => {
        ...
        it('declares the flag on bridge leaves and withholds it from filesystem-only ones', () => {
            ...
            const flagsFor = (name) => browser?.commands
...
describe('browser Session lifecycle commands', () => {
    ...
    it('keeps missing Session guidance scoped to the selected Profile', async () => {
        ...
        await expect(createProgram('', '').parseAsync(['node', 'webcmd', 'session', 'close', 'work-project-k7', '-f', 'json'])).rejects.toMatchObject({
            code: 'SESSION_NOT_FOUND',
            hint: expect.stringContaining('webcmd --profile default session list'),
        });
    });

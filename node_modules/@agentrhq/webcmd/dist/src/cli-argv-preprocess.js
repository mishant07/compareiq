 ...
 */
const ROOT_VALUE_FLAGS = new Set(['--profile', '--session', '--workspace']);
/**
...
export function rejectPositionalBrowserSessionArgv(argv) {
    const result = [...argv];
    const commandIndex = findRootCommandIndex(result);
    if (result[commandIndex] !== 'browser')
        return result;
    ...
    throw new BrowserSessionArgvError(`Browser sessions are root selectors. Use: webcmd ${replacement.join(' ')}`);
}
export function rejectMisplacedSessionSelectorArgv(argv) {
    const result = [...argv];
    const commandIndex = findRootCommandIndex(result);
    for (let index = commandIndex + 1; index < result.length; index += 1) {
        ...
        if (token === '--session' || token.startsWith('--session=')) {
            ...
            throw new BrowserSessionArgvError(`SESSION_SELECTOR_POSITION: --session must appear before the command. Use: webcmd --session ${sessionId} ${withoutMisplaced.join(' ')}`);
        }
    }
    return result;
}

...
describe('profile ensure coalescing', () => {
    const result = (alias) => ({ profile: { id: alias, displayName: alias }, created: true });
    ...
    it('does not coalesce different aliases or provider instances and retries after failure', async () => {
        ...
        const firstEnsure = vi.fn(async ({ alias }) => {
            if (alias === 'broken' && ++brokenAttempts === 1)
                throw new Error('boom');
            return result(alias);
        });
        const secondEnsure = vi.fn(async ({ alias }) => result(alias));
        const first = { ensureProfile: firstEnsure };
        const second = { ensureProfile: secondEnsure };
        await Promise.all([
...
describe('createDaemonServer', () => {
    ...
    it('handles local Session lifecycle controls outside normal dispatch', async () => {
        ...
        const listed = await postCommand(baseUrl, { id: 'list-sessions', action: 'session-list', contextId: 'profile-a' });
        expect(listed.status).toBe(200);
        await expect(listed.json()).resolves.toMatchObject({
            ok: true,
            data: [{ id: provider.sessionId, runtimeState: 'active' }],
        });
        const closed = await postCommand(baseUrl, { id: 'close-session', action: 'session-close', contextId: 'profile-a', session: provider.sessionId });
        expect(closed.status).toBe(200);
        await expect(closed.json()).resolves.toMatchObject({
            ok: true,
            data: { closed: true, alreadyIdle: false, session: provider.sessionId },
        });
        const closedAgain = await postCommand(baseUrl, { id: 'close-session-again', action: 'session-close', contextId: 'profile-a', session: provider.sessionId });

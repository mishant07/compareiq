...
async function cancelAndSettle(entries) {
...
function publicSessionHolder(holder) {
    if (!holder)
        return null;
...
function displacedSessionHolder(holder) {
    if (!holder)
        return null;
...
const UNRESOLVED_SESSION_LIFECYCLE_ACTIONS = new Set([
    'session-create',
    'session-list',
    'profile-ensure',
]);
const profileEnsures = new WeakMap();
/** @internal Exported for deterministic concurrency contract tests. */
export async function ensureProfileCoalesced(provider, alias, idempotencyKey) {
    ...
    if (existing) {
        if (existing.idempotencyKey !== idempotencyKey) {
            throw new CliError('PROFILE_ENSURE_CONFLICT', `Profile ensure for alias "${key}" is already in progress with a different idempotency key.`);
        }
    ...
    try {
        ...
        if (providerEnsures.get(key)?.operation === operation)
            providerEnsures.delete(key);
    }
}
function commandProfileId(provider, command) {
    return provider.resolveProfileId?.(command)
        ?? command.profileId
        ?? command.contextId
        ?? command.preferredContextId;
}
async function resolveBrowserSession(provider, command) {
...
async function handleSessionHandoff(provider, command, session) {
    if (!HANDOFF_ACTIONS.has(command.action))
        return null;
...
async function handleSessionLifecycle(provider, command) {
...
function abortOnResponseClose(res, controller, onAbort) {
    let completed = false;
    const onClose = () => {
        if (completed)
            return;
...
export function createDaemonServer(provider, opts) {
    const port = opts.port ?? DEFAULT_DAEMON_PORT;
    const host = opts.host ?? '127.0.0.1';
    const logBuffer = [];
    const pending = new Map();
    const leases = new SessionLeaseRegistry();
    const forceClosingRuns = new Set();
    ...
    async function handleRequest(req, res) {
        ...
        if (req.method === 'POST' && pathname === '/command') {
            try {
                ...
                const activeSessionLeases = (sessionKey) => leases.list(hasPendingWork).filter((lease) => (lease.key === sessionKey
                ...
                const commandPromise = provider.dispatch(resolvedBody, abortController.signal).finally(() => {
                    removeResponseAbort();
                    if (leaseKey && runId && !responseAborted)
                        leases.heartbeat(leaseKey, runId);
                    pending.delete(body.id);
                    if (runId && (responseAborted || forceClosingRuns.delete(runId)))
                        leases.releaseByRunId(runId);
                });

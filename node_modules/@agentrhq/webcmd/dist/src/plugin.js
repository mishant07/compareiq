...
function cloneRepoToTemp(cloneUrl) {
    const tmpCloneDir = path.join(os.tmpdir(), `webcmd-clone-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    try {
...
function retainMonorepoBaseline(repoDir, cloneDir, commitHash) {
    execFileSync('git', ['fetch', '--no-tags', cloneDir, `${commitHash}:refs/webcmd/baselines/${commitHash}`], {
        cwd: repoDir,
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe'],
    });
...
function describeGitError(error) {
    const stderr = typeof error?.stderr === 'string'
        ? error.stderr
        : error?.stderr?.toString('utf-8') ?? '';
    return stderr.trim() || error?.message || String(error);
}
/**
 ...
 */
export function getDirtyFiles(dir, options = {}) {
 ...
 */
function isInstallArtifact(line) {
    if (line.slice(0, 2) !== '??')
        return false;
...
function dirtyPathIsInside(entry, subPath) {
    const file = dirtyEntryPath(entry).replace(/\\/g, '/');
    const prefix = subPath.replace(/\\/g, '/').replace(/\/+$/, '');
    return file === prefix || file.startsWith(`${prefix}/`);
}
function formatDirtyPaths(dirty) {
    return dirty.slice(0, 10).map(describeDirtyEntry).join('\n  ');
}
function assertPluginNotDirty(name, dirty, force) {
    if (dirty.length === 0)
        return [];
    if (force)
        return dirty;
    throw new PluginError(`Plugin "${name}" has uncommitted changes that updating would destroy:\n  ${formatDirtyPaths(dirty)}`, 'Commit or stash them, re-run with --force to discard them, or develop against a symlinked checkout with "webcmd plugin install file:///path".');
}
function inspectPluginDirtiness(name, force, readDirty) {
    if (force) {
        try {
            return readDirty();
        }
...
export function looksLikePluginName(source) {
...
function installParsedPlugin(parsed, source, options = {}) {
    ...
    return withTempClone(parsed.cloneUrl, (tmpCloneDir) => {
        ...
        if (manifest?.webcmd && !checkCompatibility(manifest.webcmd)) {
            throw new Error(`Plugin requires webcmd ${manifest.webcmd}, but current version is incompatible.`);
        }
...
function installLocalPlugin(localPath, name) {
    ...
    if (manifest?.webcmd && !checkCompatibility(manifest.webcmd)) {
        throw new PluginError(`Plugin requires webcmd ${manifest.webcmd}, but current version is incompatible.`, 'Upgrade webcmd to a compatible version.');
    }
    const pluginName = manifest?.name ?? name;
    const targetDir = path.join(PLUGINS_DIR, pluginName);
    if (fs.existsSync(targetDir)) {
...
function installMonorepo(cloneDir, cloneUrl, repoName, manifest, subPlugin, installAll = false) {
    ...
    if (!subPlugin && !installAll) {
        throw new PluginError(`This source has ${pluginsToInstall.length} plugins; install one with github:user/repo/<plugin>, or pass --all to install every plugin.`);
    }
    ...
    postInstallMonorepoLifecycle(repoRoot, eligiblePlugins.map((p) => resolveRepoContainedPath(repoRoot, p.entry.path)));
    publishMonorepoPlugins(repoDir, PLUGINS_DIR, publishPlugins, publishRepo, (commitHash) => {
...
function collectUpdatedMonorepoPlugins(monoName, lock, manifest, cloneUrl, tmpCloneDir, only) {
    const updatedPlugins = [];
    for (const [pluginName, entry] of Object.entries(lock)) {
        if (only && pluginName !== only)
            continue;
 ...
 */
export function updatePlugin(name, options = {}) {
    const targetDir = path.join(PLUGINS_DIR, name);
    if (!fs.existsSync(targetDir)) {
        throw new Error(`Plugin "${name}" is not installed.`);
    }
    ...
    if (source?.kind === 'monorepo') {
        ...
        return withTempClone(cloneUrl, (tmpCloneDir) => {
            ...
            const updatedPlugins = collectUpdatedMonorepoPlugins(monoName, lock, manifest, cloneUrl, tmpCloneDir, name);
            ...
            if (commitHash)
                retainMonorepoBaseline(monoDir, tmpCloneDir, commitHash);
            publishMonorepoPlugins(monoDir, PLUGINS_DIR, [{ name: plugin.name, subPath: plugin.manifestEntry.path }], {
 ...
 */
export function findOverridesNeedingReconcile(pluginNames) {
    const homeDir = getHomeDir();
    const records = readOverrideRecords(homeDir);
    const needs = [];
    for (const [commandKey, record] of Object.entries(records)) {
        if (pluginNames && !pluginNames.includes(record.plugin))
            continue;
        ...
        if (!fs.existsSync(record.sourcePath))
            continue;
        if (fileSha256(record.sourcePath) === record.sourceSha256)
            continue;
        needs.push({
            commandKey,
 ...
 */
function linkHostWebcmd(pluginDir) {
    try {
...
function getHostSourcePackageExports(hostRoot) {
    const pkg = JSON.parse(fs.readFileSync(path.join(hostRoot, 'package.json'), 'utf-8'));
...
function ensureSourcePackageShim(targetLink, hostRoot) {
    ...
    try {
        if (fs.readlinkSync(srcLink) === srcTarget)
            needsSrcLink = false;
    }
    catch { }
    if (needsSrcLink) {
...
function ensureHostWebcmdPackageLink(targetLink, hostRoot) {
    ...
    try {
        if (fs.readlinkSync(targetLink) === hostRoot)
            needsUpdate = false;
    }
    catch { }
    if (!needsUpdate)
...
function resolveHostWebcmdRoot(startFile = fileURLToPath(import.meta.url)) {

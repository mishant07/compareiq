...
describe('auth command format validation', () => {
    it.each(['status', 'refresh'])('does not run %s probes for an unsupported format', async (subcommand) => {
        registerWhoami('alpha', { quick: true, quickLoggedIn: true });
...
describe('auth verbose flag', () => {
    ...
    it.each(['status', 'refresh'])('turns on verbose mode for auth %s', async (subcommand) => {
        delete process.env.WEBCMD_VERBOSE;
        registerWhoami('alpha', { quick: true, quickLoggedIn: true });
        const program = new Command('webcmd');
        registerAuthCommands(program);
    ...
    it.each(['status', 'refresh'])('leaves verbose mode off for auth %s without the flag', async (subcommand) => {
        delete process.env.WEBCMD_VERBOSE;
        registerWhoami('alpha', { quick: true, quickLoggedIn: true });
        const program = new Command('webcmd');
        registerAuthCommands(program);

...
describe('convention audit', () => {
    ...
    function makeProject(manifest, files) {
        const root = fs.mkdtempSync(path.join(os.tmpdir(), 'webcmd-convention-audit-'));
        tempDirs.push(root);
...
describe('extension removal audit', () => {
    it('does not ship the old browser extension package', () => {
        expect(fs.existsSync(path.join(process.cwd(), 'extension', 'manifest.json'))).toBe(false);
    });
    it('does not keep extension-only e2e smoke tests', () => {
        expect(fs.existsSync(path.join(process.cwd(), 'tests/e2e/browser-ax-chrome.test.ts'))).toBe(false);
    });
});

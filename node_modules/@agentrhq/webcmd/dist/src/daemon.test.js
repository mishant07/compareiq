...
import { COMMAND_RESULT_UNKNOWN_CODE, COMMAND_RESULT_UNKNOWN_HINT, buildCommandDispatchFailure, buildCommandTimeoutFailure, buildExtensionDisconnectFailure, commandResultUnknownMessage, getResponseCorsHeaders, } from './daemon-utils.js';
...
describe('getResponseCorsHeaders', () => {
    ...
    it('does not add CORS headers for command endpoints even from the packaged runtime origin', () => {
...
describe('sweepOrphanedRegistrationTriggers', () => {
    it('terminates any Chrome process carrying --no-startup-window, since only webcmd ever passes that flag', async () => {
        const findProcesses = vi.fn(async (platform) => {

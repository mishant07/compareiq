...
export function runConventionAudit(opts) {
    ...
    const manifest = manifestPaths.flatMap(manifestPath => JSON.parse(fs.readFileSync(manifestPath, 'utf-8')));
    ...
    const readSource = (logicalPath) => {
        if (!allowedPaths.has(logicalPath))
            return undefined;
        ...
        if (sourcePath !== projectRoot && !sourcePath.startsWith(rootPrefix))
            return undefined;
        try {
            const realSourcePath = fs.realpathSync(sourcePath);
            if (realSourcePath !== projectRoot && !realSourcePath.startsWith(rootPrefix))
                return undefined;
    ...
    const targetExists = !target || commands.some(command => (target.includes('/') ? command.command === target : command.site === target));
...
export function auditAdapterConventions(commands, options) {
...
function toAdapterAnalysisCommand(entry) {
...
function auditColumnDrop(command, entry, source, logicalPath) {
...
function auditTypedErrorPatterns(command, source, logicalPath) {
    const violations = [];

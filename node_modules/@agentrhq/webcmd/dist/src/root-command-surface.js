...
import { structuralErrorFromCommander } from './command-surface.js';
...
export function configureHostedWorkspaceOption(program) {
...
export function parseHostedRootCommandSurface(argv) {
    ...
    const { profile, session, workspace } = root.opts();
...
export function rootCompletionSentinelIndex(argv) {
...
function findRootCommandBoundary(argv) {
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (token.startsWith('--profile=') || token.startsWith('--session=') || token.startsWith('--workspace='))
            continue;
        if (token === '--')

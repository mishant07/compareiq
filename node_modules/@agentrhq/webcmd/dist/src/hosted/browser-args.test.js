...
describe('hosted browser argument surface', () => {
    it('requires an opaque root session selector for raw browser dispatch', () => {
        expect(() => validateRawBrowserSession(undefined)).toThrowError(expect.objectContaining({ code: 'SESSION_REQUIRED', exitCode: 2 }));
        expect(() => validateRawBrowserSession('work')).toThrowError(expect.objectContaining({ code: 'INVALID_SESSION_SELECTOR', exitCode: 2 }));
    });
    it('includes the selected profile in raw-session recovery commands', () => {
        expect(() => validateRawBrowserSession(undefined, 'work')).toThrowError(expect.objectContaining({

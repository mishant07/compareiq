...
import { VirtualFileMissingError, realHostedFileIo } from './file-io.js';
export async function prepareHostedFiles(input, io = realHostedFileIo) {
    ...
    for (const upload of inputs) {
        ...
        if (uploaded.reference.$webcmdArtifact.id)
            inputIdByPath.set(`${upload.argument}\0${upload.localPath}`, uploaded.reference.$webcmdArtifact.id);
    }
    ...
    const mutableByArgument = new Map();
...
export async function materializeHostedOutputs(input, io = realHostedFileIo) {
...
function resolveLocalPath(cwd, value, argName, io) {
    if (!value.trim()) {
...
function resolveOutputPath(cwd, value, argName, io) {
    if (!value.trim()) {
...
function resolveUserPath(cwd, value, io) {
    if (io !== realHostedFileIo)
        return value;
    if (value === '~')
        return homedir();
    if (value.startsWith('~/'))
...
async function readLocalFile(localPath, argName, io) {
    try {
...
function targetPathForReceipt(output, receipt, seenRelativePaths, io) {

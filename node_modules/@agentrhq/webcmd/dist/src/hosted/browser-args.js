...
import { BROWSER_RUN_HELP_TEXT, browserCommandCatalog, browserOptionFlags, browserOptionValueParser, } from '../browser/command-catalog.js';
...
export function parseHostedBrowserStructure(argv) {
    ...
    for (const contract of browserCommandCatalog) {
        ...
        if (contract.command === 'run') {
            ...
            leaf.helpInformation = ((contextOptions) => (originalHelpInformation(contextOptions) + BROWSER_RUN_HELP_TEXT));
        }
...
function validateBindSelectors(result) {
    if (result.commandName !== 'bind')
        return;
    ...
    if (Boolean(page) === Boolean(targetId)) {
        throw new CommanderStructuralError(`error: Bind requires exactly one of --page or --target-id\n`, EXIT_CODES.USAGE_ERROR);
    }
}

...
export function setEndpoint(input) {
    return updateJson(input.site, 'endpoints.json', input, (endpoints) => {
        ...
        if (input.params !== undefined)
            endpoint.params = input.params;
        if (input.rowsPath !== undefined)
            endpoint.rows_path = input.rowsPath;
        if (input.sampleFields !== undefined)
            endpoint.sample_fields = input.sampleFields;
...
export function markEndpointStale(input) {
    return updateJson(input.site, 'endpoints.json', input, (endpoints) => {
        const endpoint = objectValue(endpoints[input.name]);
        if (!endpoint)
            throw statusError(404, `Endpoint ${input.name} was not found.`);
        endpoints[input.name] = { ...endpoint, stale: true };
        return endpoints;
    });
}
export function addFieldMapping(input) {
...
export async function getVerifyFixture(site, command, opts = {}) {
...
export async function showSiteMemory(site, opts = {}) {
...
export async function listSiteMemory(site, opts = {}) {
    const root = siteRoot(site, opts);
    const paths = await memoryPaths(root, opts.paths);
...
async function updateText(site, path, opts, update) {
...
async function updateJson(site, path, opts, update) {
    const root = await ensureSiteRoot(site, opts);
    const target = join(root, path);
    return withWriteLock(target, async () => {
...
export async function writeProductFile(productKey, path, body, opts = {}) {
    const productRoot = join(sitesRoot(opts), productSegment(productKey));
...
export async function deleteProductFile(productKey, path, opts = {}) {
    const productRoot = join(sitesRoot(opts), productSegment(productKey));
    const relative = containedRelativePath(productRoot, path);
    try {
...
export async function copyDraftFiles(productKey, taskId, paths, opts = {}) {
...
export function sitesRoot(opts = {}) {
    return join(requiredHomeDir(opts), '.webcmd', 'sites');
}
export function containedRelativePath(root, path) {
    return safeRelativePath(root, path).split(sep).join('/');
}
async function writeSiteFile(site, path, body, opts) {
    const root = await ensureSiteRoot(site, opts);
    const target = join(root, path);
    await mkdir(dirname(target), { recursive: true });
...
async function ensureSiteRoot(site, opts) {
    const root = siteRoot(site, opts);
...
async function walkFiles(root, dir = root) {
    ...
    const nested = await Promise.all(entries.map(async (entry) => {
        ...
        if (entry.isDirectory())
            return entry.name === 'candidates' ? [] : walkFiles(root, path);
        if (entry.isFile() && !isInternalWritePath(entry.name))
            return [relative(root, path)];
        return [];
    }));
    return nested.flat();
}
async function readableRelativePath(root, path) {
    const relativePath = safeRelativePath(root, path);
    if ((await lstat(join(root, relativePath))).isSymbolicLink())
...
function validateVerifyFixture(body) {
    let fixture;
    try {
...
async function assertInsideSiteRoot(root, parent, path) {

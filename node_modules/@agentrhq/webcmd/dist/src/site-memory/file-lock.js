...
export async function withFileLock(target, fn, options = {}) {
    const lockPath = lockPathFor(target);
    const owned = heldLocks.getStore();
    if (owned?.has(lockPath))
        return fn();
...
async function acquire(lockPath, options) {
    ...
    let attempt = 0;
    for (;;) {
        if (await create(lockPath, body))
            return token;
        if (await breakIfAbandoned(lockPath, staleMs))
            continue;
        if (Date.now() >= deadline)
            throw busyError(lockPath, await readOwner(lockPath), timeoutMs);
        await delay(backoffMs(attempt++));
    }
}
/** Resolves true when this call created the lock, false when someone else holds it. */
async function create(lockPath, body) {
    let handle;
    for (let attempt = 0;; attempt += 1) {
        try {
            ...
            if (isNodeError(err) && err.code === 'EEXIST')
                return false;
            if (process.platform !== 'win32' || !isNodeError(err) || err.code !== 'EPERM' || attempt >= 2)
                throw err;
            await delay(backoffMs(attempt));
        }
    }
 ...
 * left alone.
 */
async function breakIfAbandoned(lockPath, staleMs) {
...
async function release(lockPath, token) {
    const owner = await readOwner(lockPath);
    if (owner.token !== undefined && owner.token !== token)
        return;
    await unlink(lockPath).catch(ignoreMissing);
}
...
function busyError(lockPath, owner, timeoutMs) {
    ...
    return new CliError('SITE_MEMORY_BUSY', `Site memory ${file} is locked by another webcmd process${holder}.`, `Nothing was written. Wait for that process to finish and retry — it has held the lock for over ${Math.round(timeoutMs / 1000)}s.${stop}`, EXIT_CODES.TEMPFAIL);
}

...
import { addOutputFormatOption, applyUnknownOptionContract, CommanderStructuralError, ensureOutputFormatOptions, outputFormatIsExplicit, resolveCommandOutputFormat } from './command-surface.js';
...
import { buildRootHelpPresentation, classifyAdapter, commanderCommandHelpData, installCommanderNamespaceStructuredHelp, installRootPresentationHelp, installStructuredHelp, rootHelpData } from './help.js';
import { EXIT_CODES, getErrorMessage, CliError, ArgumentError } from './errors.js';
...
function selectedProfileProviderLabel() {
...
const externalRootCommands = new WeakSet();
function parsePositiveIntOption(value, _label, fallback) {
    const parsed = value === undefined ? fallback : Number.parseInt(value, 10);
    return Number.isSafeInteger(parsed) && parsed > 0 ? parsed : fallback;
}
function parseSessionListLimit(value) {
...
function isInteractiveSkillCommand(opts) {
    return !opts.json && process.stdin.isTTY === true && process.stdout.isTTY === true;
}
async function resolveSkillCommandOptions(opts, questions) {
    if (!isInteractiveSkillCommand(opts))
        return opts;
...
async function resolveSkillRemoveOptions(opts) {
    return resolveSkillCommandOptions(opts, {
...
function wantsJsonEnvelope(opts) {
    return opts.json === true || opts.format === 'json';
}
async function handleSkillRemoveCommand(action, json) {
    try {
        const result = await action();
        if (json) {
            console.log(JSON.stringify(result, null, 2));
...
async function captureNetworkItems(page) {
    ...
    const raw = await page.evaluate(`(function(){ var out = window.__webcmd_net || []; window.__webcmd_net = []; return JSON.stringify(out); })()`);
    try {
...
function getBrowserSession(command) {
    return validateRawBrowserSession(getCommandOption(command, 'session'), getCommandOption(command, 'profile'));
}
 ...
 */
function withBrowserVerbose(command) {
 ...
 */
async function emitActionResult(command, payload, human) {
    if (!outputFormatIsExplicit(command)) {
        human();
        return;
    }
    const fmt = resolveCommandOutputFormat(command, command.opts().format);
    if (fmt === null)
        return;
    await renderOutput(payload, { fmt, fmtExplicit: true });
}
async function handleAdapterOverride(commandKey, _opts, command) {
...
export function createProgram(BUILTIN_CLIS, USER_CLIS, pluginsDir = PLUGINS_DIR) {
    ...
    const listCmd = configureListCommandSurface(program.command('list'))
        ...
        const presentation = commandListPresentation(filterCommandsByTag([...new Set(getRegistry().values())].map((command) => {
            ...
            const origin = classified.kind === 'local' && overrides[commandKey]
                ? { kind: 'override', plugin: overrides[commandKey].plugin }
                : classified;
    ...
    validateCmd.action(async (target, opts) => {
        ...
        if (fmt === 'table')
            ...
            await renderOutput(report, { fmt, fmtExplicit });
    ...
    verifyCmd.action(async (target, opts) => {
        ...
        if (fmt === 'table')
            ...
            await renderOutput(r, { fmt, fmtExplicit });
    ...
    const renderSkillsList = (fmt, fmtExplicit, source) => renderOutput(listWebcmdSkills(), {
        fmt,
        fmtExplicit,
    ...
    program
        ...
        if (runtimeNotice)
            process.stdout.write(runtimeNotice);
    ...
    sessionCreateCmd.action(async (name, opts, command) => {
    ...
    sessionListCmd.action(async (opts, command) => {
        ...
        if (status?.runtimeConnected && !isDaemonStale(status, PKG_VERSION)) {
            rows = await sendCommand('session-list', { contextId: profileId, limit: opts.limit });
        }
        else {
    ...
    sessionCloseCmd.action(async (positionalSessionId, opts, command) => {
        const fmt = resolveCommandOutputFormat(command, opts.format);
        if (fmt === null)
            return;
        const profileId = getSelectedProfileId(command);
        ...
        if (!sessionId) {
            throw new ArgumentError('Missing Session ID.', `Use \`${CLI_COMMAND} session close <session-id>\` or \`${CLI_COMMAND} --session <session-id> session close\` with a readable Session ID.`);
        }
...
Test either local adapter: ${CLI_COMMAND} browser verify <site>/<command>`)
        .action(async (name, _opts, initCmd) => {
        try {
...
`;
    ...
    browserVerifyCmd.action(async (name, opts = {}) => {
        ...
        try {
            ...
            if (!fs.existsSync(filePath)) {
                ...
                if (asTable) {
                    ...
                    await renderOutput({ ok: false, site, command, error: { code: 'ADAPTER_NOT_FOUND', message, hint } }, { fmt, fmtExplicit });
                process.exitCode = EXIT_CODES.GENERIC_ERROR;
                return;
            }
    ...
    function rawBrowserAction(fn) {
        return async (opts, command) => {
            ...
            if (fmt === null)
                return;
            const emit = (payload) => renderOutput(payload, { fmt, fmtExplicit: true });
            try {
    ...
    browser.addCommand(withBrowserVerbose(new Command('bind')
        ...
        if (page && targetId)
            throw new BrowserCommandError('Use either --page or --target-id, not both', 'invalid_request');
        if (!page && !targetId)
            throw new BrowserCommandError('Bind requires a non-empty --page or --target-id', 'invalid_request');
    ...
    doctorCmd.action(async (opts) => {
        ...
        if (doctorRequiredChecksFailed(report))
    ...
    function printReconcileReport(needs) {
        if (needs.length === 0)
            return;
        console.log();
    ...
    catalogListCmd.action(async (opts) => {
    ...
    catalogAddCmd.action(async (source, opts) => {
    ...
    pluginSearchCmd.action(async (query, opts) => {
    ...
    adapterStatusCmd.action(async (opts) => {
        ...
        try {
            ...
            if (!userClisListed && err.code === 'ENOENT') {
                if (fmt !== 'table')
                    renderOutput([], { fmt, fmtExplicit: outputFormatIsExplicit(adapterStatusCmd) });
                else
                    console.log('No local adapters installed.');
    ...
    adapterCmd
        ...
        .action(async (site, opts, command) => {
        if (opts.all) {
            let userClisListed = false;
            try {
                const userEntries = await fs.promises.readdir(USER_CLIS, { withFileTypes: true });
    ...
    const localAdapterPath = (commandKey, commandName) => {
        const parsed = splitAdapterCommandKey(commandKey, commandName);
        if (!parsed) {
            throw new ArgumentError('Adapter command must use site/command format.', 'Example: webcmd adapter path quotes/list  (also accepts: webcmd adapter path quotes list)');
        }
        const key = `${parsed.site}/${parsed.command}`;
        const registered = getRegistry().get(key);
        const source = registered && resolveAdapterSourcePath(registered);
        if (!source)
            throw new ArgumentError(`Adapter source is unavailable for ${key}.`);
        return source;
    };
    const reportLocalAdapterPath = async (command, commandKey, commandName) => {
        const parsed = splitAdapterCommandKey(commandKey, commandName);
    ...
    adapterSourceCmd.command('get').description('Print local source path; --output is hosted-only').argument('<command>').argument('[name]').option('-o, --output <path>').action(async (commandKey, commandName, options, command) => {
        const parsed = splitAdapterCommandKey(commandKey, commandName);
        const key = parsed ? `${parsed.site}/${parsed.command}` : commandKey;
        if (options.output)
            throw new ArgumentError(`Local adapter source get does not support --output. Use webcmd adapter path ${key} and edit that file.`);
        await reportLocalAdapterPath(command, commandKey, commandName);
    });
    adapterSourceCmd.command('put').description('Hosted-only source write; local users edit the adapter path').argument('<command>').argument('<path>').action((commandKey) => {
    ...
    adapterCmd.command('path')
        ...
        .action((commandKey, commandName, _opts, command) => reportLocalAdapterPath(command, commandKey, commandName));
    ...
    profileListCmd.action(async (opts, command) => {
        const fmt = resolveCommandOutputFormat(command, opts.format);
        if (fmt === null)
            return;
        const status = await fetchDaemonStatus();
        ...
        if (fmt !== 'table') {
            ...
            if (!daemonUsable) {
                const error = new CliError('DAEMON_UNAVAILABLE', status
                    ? `Daemon ${formatDaemonVersion(status)} is stale for CLI v${PKG_VERSION}; profile list is incomplete.`
                    : 'Daemon is not running; profile list is incomplete.', status ? 'Run: webcmd daemon restart' : 'Run webcmd doctor after opening Chrome.');
    ...
    profileCmd
        ...
        .action(async (alias, _opts, command) => {
        const provider = selectedProfileProvider();
        if (provider === 'slab') {
            let pending = await prepareSlabProfileEnsure(alias);
            let ensured;
            try {
                ensured = await sendCommand('profile-ensure', pending);
            }
            catch (error) {
                if (!(error instanceof BrowserCommandError) || error.code !== 'PROFILE_REPAIR_REQUIRED')
                    throw error;
    ...
    profileCmd
        ...
        .argument('<alias>', 'Local alias, e.g. work or personal')
        .action(async (contextId, alias, _opts, command) => {
        try {
    ...
    profileCmd
        ...
        .action(async (profile, _opts, command) => {
        const status = await fetchDaemonStatus();
        const config = loadProfileConfig();
        const connected = status && !isDaemonStale(status, PKG_VERSION) && Array.isArray(status.profiles)
            ? status.profiles
    ...
    installStructuredHelp(runCommand, () => commanderCommandHelpData(browser, runCommand, { globalCommand: program }), BROWSER_RUN_HELP_TEXT);
    ...
    for (const namespace of program.commands) {
        if (namespace.commands.length === 0 || !SUGGEST_NAMESPACES.has(namespace.name()))
            continue;
        namespace.on('command:*', (operands) => {
 ...
 */
async function installSourceSuggestion(err, source) {
    const { InvalidPluginSourceError, looksLikePluginName } = await import('./plugin.js');
    if (!(err instanceof InvalidPluginSourceError) || !looksLikePluginName(source))
        return undefined;
    try {
        const { readCatalog, searchCatalogPlugins } = await import('./plugin-catalog.js');
...
export async function loadAntigravityServe(pluginsDir = PLUGINS_DIR) {
 ...
 */
export function isExternalRootCommand(program, name) {
    const command = program.commands.find(candidate => candidate.name() === name);
    return command !== undefined && externalRootCommands.has(command);
}
export async function runCli(BUILTIN_CLIS, USER_CLIS, program = createProgram(BUILTIN_CLIS, USER_CLIS)) {

...
import { withAdapterRepairHelp } from './errors.js';
...
export function errorEnvelopeFormat(fmt) {
    return typeof fmt === 'string' && fmt.trim().toLowerCase() === 'json' ? 'json' : 'yaml';
}
export function requestedFormatFromArgv(argv) {
    let format;
 ...
 */
export function requestedMachineFormat(argv) {
...
export function formatErrorEnvelope(envelope, opts = {}) {
    ...
    if (fmt === 'json')
        return formatOutput(repaired, { fmt: 'json', fmtExplicit: true });

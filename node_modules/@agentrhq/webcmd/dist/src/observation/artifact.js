...
function baseWebcmdDir() {
    return process.env.WEBCMD_CONFIG_DIR || path.join(os.homedir(), '.webcmd');
}
function safeSegment(value) {
    const safe = (value || 'default').replace(/[^a-zA-Z0-9_-]+/g, '_');
    return safe || 'default';
}
export function getTraceDirectory(contextId, traceId, baseDir = baseWebcmdDir()) {
...
function renderSummary(session, events, opts) {
    ...
    const lines = [
        '---',
        'schemaVersion: 1',
        `webcmdVersion: ${yamlScalar(PKG_VERSION)}`,
